"use client";

import InsuranceModel, { InsuranceDTO } from "../_models/insurance";
import PageList from "../_models/page-list";
import { apiRequest, SimpleResponse } from "../_utils/axios";

const endpoint = "/admin/information/insurance-claims";

export async function getInsurances(
  page: number,
  limit: number = 10
): Promise<SimpleResponse<PageList<InsuranceModel>>> {
  const { data, error } = await apiRequest<PageList<InsuranceDTO>>(endpoint, {
    method: "GET",
    params: { page, limit },
  });

  if (error) {
    return { data: undefined, error };
  }

  // DTO 데이터를 Model로 변환
  const newData: PageList<InsuranceModel> = {
    ...data!,
    data: data!.data.map((insurance) => InsuranceModel.fromJson(insurance)),
  };

  return { data: newData, error };
}

export async function addInsurance(insurance: InsuranceModel) {
  const { data, error } = await apiRequest<InsuranceDTO>(endpoint, {
    method: "POST",
    data: insurance.toJson(),
  });

  if (error) {
    return { data: undefined, error };
  }

  return { data: InsuranceModel.fromJson(data!), error };
}

export async function updateInsurance(insurance: InsuranceModel) {
  const { data, error } = await apiRequest<InsuranceDTO>(
    `${endpoint}/${insurance.id}`,
    {
      method: "PUT",
      data: insurance.toJson(),
    }
  );

  if (error) {
    return { data: undefined, error };
  }

  return { data: InsuranceModel.fromJson(data!), error };
}

export async function deleteInsurance(id: string) {
  const { error } = await apiRequest(`${endpoint}/${id}`, {
    method: "DELETE",
  });
  return { error };
}
